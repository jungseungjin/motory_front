...
const BottomContainer_Button_Text = styled.Text`
  ...
  color: #ffffff;
`;
function Question_revise({navigation, route}) {
  console.log(route.params[1]);
  const [isLoading, setIsLoading] = React.useState(false);
  const [title, setTitle] = React.useState(route.params[1].title);
  const [contents, setContents] = React.useState(route.params[1].contents);
  const revise = async function () {
    try {
      setIsLoading(true);
      let url = Domain + 'question_revise';
      let data = {
        iq_id: route.params[1]._id, //글의 아이디값
        title: title, //제목
    ...
    <Container>
      ...
      <BottomContainer>
        <BottomContainer_Button
          ...
          <BottomContainer_Button_Text>수정완료</BottomContainer_Button_Text>
        </BottomContainer_Button>
      </BottomContainer>
...
export default Question_revise;
